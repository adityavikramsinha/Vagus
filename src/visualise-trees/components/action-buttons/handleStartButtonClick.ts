...
const validateAndRun = () => {
    ...
    if (startId === NOTSET) return Exception.START_NODE_NOTSET;
    if (endId === NOTSET) return Exception.END_NODE_NOTSET;

    const activeTsFile = useTreeStore.getState().activeFiles.ts;

    match(activeTsFile)
        .with('ts-1', async () =>
            runAlgorithmAnimation(AlgoType.BREADTH_FIRST_SEARCH, startId, endId),
        )
        ...
        .with('ts-4', async () =>
            runAlgorithmAnimation(AlgoType.DEPTH_FIRST_SEARCH, startId, endId),
        )
        .with('ts-5', async () =>
            runAlgorithmAnimation(AlgoType.ZERO_ONE_BREADTH_FIRST_SEARCH, startId, endId),
        );
    return null;
};
