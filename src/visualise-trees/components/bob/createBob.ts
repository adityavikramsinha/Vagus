...
const createBob = (
    ...
    return {
        ...
        onUpdate: (index: string, newX: number, newY: number) => {
            if (
                useTreeStore.getState().nodes[index] &&
                useTreeStore.getState().nodes[index].isDragging
            ) {
        ...
        onDragChange: (index: string, dragging: boolean) => {
            if (useTreeStore.getState().nodes[index])
                useTreeStore.getState().nodes[index].isDragging = dragging;
        },
        isDragging: false,
        containerRef,
    };
