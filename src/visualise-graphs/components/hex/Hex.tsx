 ...
 */
const handleHexClick = (visited: typeof NOTSET | NodeType, pathNode: boolean, isStartNode: boolean, isEndNode: boolean, isBombNode: boolean, isWeightNode: boolean, isWallNode: boolean, id: string | typeof NOTSET) => {
    ...
    if (isStartNode) {
        ...
        .with('io-4',
            () => changeNode(NodeType.WEIGHT_NODE, NodeAction.SET, id))
...
const Hex: React.FC<HexProps> = ({x, y, id}) => {
    ...
    const visited = useGraphStore(
        state => state.visitedNodes.has(id) ? state.visitedNodes.get(
            id) : NOTSET);
    const pathNode = useGraphStore(state => state.pathNodes.has(id));
    const isRandomAlgorithmSteppingStone = useGraphStore(
        state => state.randomPathId === id);
    ...
    return (
        <div onClick={() => handleHexClick(visited, pathNode, isStartNode,
            isEndNode, isBombNode, isWeightNode, isWallNode, id)}
            ...
            <div className={cn(
                ...
                {isStartNode && <NodeIcon.StartNodeIcon
					className="group-hover:animate-hex-hover origin-center"/>}
