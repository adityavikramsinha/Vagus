...
const HexBoard: React.FC = () => {
    ...
    const { HEX_WIDTH, HEX_HEIGHT } = useGraphStore((state) => state.hexDimensions);
    ...
    React.useEffect(() => {
        const handleResize = () => {
            ...
            Syncer.setGraph(useGraphStore.getState().hexes).connectGraph(rows, cols);
            setLoading(false);
            const startPosition = Math.floor(rows * cols * 0.25);
            const endPosition = Math.floor(rows * cols * 0.75);
    ...
    return (
        <div
            className="relative overflow-hidden w-[79vw] flex content-center justify-center"
            id="hex-board"
        >
            {isLoading ? (
                <Loading />
            ) : (
                hexes.map(({ x, y, id }) => <Hex x={x} y={y} id={id} key={id} />)
            )}
        </div>
    );
};
