 ...
 */
const performStartButtonJobs = (algoFile: string | NOTSET_t): Exception => {
    ...
    else if (bombNodeId !== NOTSET && algoFile === 'ts-10')
        return Exception.BI_DIRECTIONAL_EXTRA_ARGS;
    ...
    for (const [id, nodeType] of Object.entries(useGraphStore.getState().hexBoard)) {
        match(nodeType)
            ...
            .with(NodeType.WEIGHT_NODE, () => {
                ...
                srcNode.getAdjVertices().forEach((edge) => Syncer.updateEdge(edge.dest, id));
            })
            .otherwise(() => {});
    }
    ...
    match(algoFile)
        .with(
            P.union('ts-1', 'ts-7'),
            async () =>
                await runAlgorithmAnimation(AlgoType.A_STAR_SEARCH, {
                    startNodeId,
                    endNodeId,
                    bombNodeId,
                }),
        ...
        .with(
            P.union('ts-2', 'ts-6'),
            async () =>
                await runAlgorithmAnimation(AlgoType.BEST_FIRST_SEARCH, {
                    startNodeId,
                    endNodeId,
                    bombNodeId,
                }),
        ...
        .with(
            'ts-3',
            async () =>
                await runAlgorithmAnimation(AlgoType.BREADTH_FIRST_SEARCH, {
                    startNodeId,
                    endNodeId,
                    bombNodeId,
                }),
        ...
        .with(
            'ts-4',
            async () =>
                await runAlgorithmAnimation(AlgoType.DEPTH_FIRST_SEARCH, {
                    startNodeId,
                    endNodeId,
                    bombNodeId,
                }),
        ...
        .with(
            'ts-8',
            async () =>
                await runAlgorithmAnimation(AlgoType.DIJKSTRAS_SEARCH, {
                    startNodeId,
                    endNodeId,
                    bombNodeId,
                }),
        ...
        .with(
            'ts-9',
            async () =>
                await runAlgorithmAnimation(AlgoType.BELLMAN_FORD, {
                    startNodeId,
                    endNodeId,
                    bombNodeId,
                }),
...
export const handleStartButtonClick = (): StartButtonError | null => {
    ...
    return match(err)
        .with(Exception.START_NODE_NOTSET, () => ({
            ...
            fix:
                'This exception is thrown by the Runtime Environment because no Start Node has' +
        ...
        .with(Exception.END_NODE_NOTSET, () => ({
            ...
            fix:
                'This exception is thrown by the Runtime Environment because no End Node has' +
