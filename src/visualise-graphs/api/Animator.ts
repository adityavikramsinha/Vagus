...
export default class Animator {
    ...
    static async animateVisitedNodes(
        visited: Map<string, NodeType.START_NODE | NodeType.BOMB_NODE>,
    ) {
        const store = useGraphStore.getState();
    ...
    static async animateRandomWalk(nodeId: string | NOTSET_t): Promise<void> {
        if (
            useGraphStore.getState().endId === nodeId ||
            BackendStateManager.graph()
                .vertices()
                .get(nodeId as string)
                .getAdjVertices().size === 1
        ) {
